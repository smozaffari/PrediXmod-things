...
obs_2 <- obs_1[order(rownames(obs_1)),]	#sort genes in rows

obs_3 <- obs_2[which(rownames(obs_2)%in%rownames(pred_2)),]		#keep overlap of predicted genes in new observed table
pred_3 <- pred_2[which(rownames(pred_2)%in%rownames(obs_2)),]	#keep overlap of observed genes in new predicted table
