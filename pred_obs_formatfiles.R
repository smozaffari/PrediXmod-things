...
if (duplicates > 0) {
   ...
   for (i in 1:length(duplicates)) {
       ...
       filename <- paste(ensembl, tissue, obs_3$Gene[duplicates[i]], duplicates[i], sep="_");			#new filename to output table
       print (paste("There are 2 Ensembl IDs for 1 gene: ", obs_3$Gene[duplicates[i]], sep = "")) ; 	#notify which gene is duplicated
       var_name <- paste(obs_3$Gene[duplicates[i]], duplicates[i], tissue,  sep="_");	       	                #new name 
  ...
  obs_4 <- obs_3[,-c(which(!colnames(obs_3)%in%colnames(pred_3)))]
    new_name <- paste(ensembl, tissue, "unique", sep = "_")
